...
// MIGHTY GORGON - BEGIN
// Before using this class, make sure you have defined and instantiated $auth properly
// MIGHTY GORGON - END
...
class umil
{
	...
	*/
	...
	/**
	* This will hold the text output for the result of the command.  $lang['SUCCESS'] if everything worked.
	*
	* @var string
	*/
	var $result = '';
	...
	function get_output_text()
	{
		...
		if (sizeof($args))
		{
			...
			if (sizeof($args))
			{
				...
				foreach ($args as $arg)
				{
					$lang_args[] = (isset($lang[$arg])) ? $lang[$arg] : $arg;
				}

				return @vsprintf(((isset($lang[$lang_key])) ? $lang[$lang_key] : $lang_key), $lang_args);
			}
			else
			{
				return ((isset($lang[$lang_key])) ? $lang[$lang_key] : $lang_key);
			}
		}
	...
	*/
	function run_actions($action, $versions, $version_config_name, $version_select = '')
	{
		...
		elseif ($action == 'uninstall' && $db_version)
		{
	...
	function _call_custom_function($functions, $action, $version)
	{
		...
		foreach ($functions as $function)
		{
			if (function_exists($function))
			{
				...
				elseif (is_array($returned) && isset($returned['command']))
				{
	...
	* @return result
	*/
	function config_add($config_name, $config_value = '', $clear_cache = true)
	{
		// Multicall
		...
		set_config($config_name, $config_value, $clear_cache);
	...
	* @return result
	*/
	function config_update($config_name, $config_value = '', $clear_cache = true)
	{
		// Multicall
		if ($this->multicall(__FUNCTION__, $config_name))
		{
	...
	*/
	function module_add($class, $parent = 0, $data = array(), $include_path = false)
	{
		...
		if (!isset($data['module_langname']))
		{
			...
			$info_file = "$class/info/{$class}_$basename." . PHP_EXT;

			// The manual and automatic ways both failed...
			if (!file_exists((($include_path === false) ? (IP_ROOT_PATH . 'includes/') : $include_path) . $info_file))
			{
			...
			{
				include((($include_path === false) ? (IP_ROOT_PATH . 'includes/') : $include_path) . $info_file);
			}
		...
		// The "manual" way
		$this->umil_start('MODULE_ADD', $class, ((isset($lang[$data['module_langname']])) ? $lang[$data['module_langname']] : $data['module_langname']));
		...
		elseif (!$this->module_exists($class, false, $parent))
		{
			return $this->umil_end('PARENT_NOT_EXIST');
		}
		...
		{
			include(IP_ROOT_PATH . 'includes/acp/acp_modules.' . PHP_EXT);
	...
	*/
	function permission_add($auth_option, $global = true)
	{
		...
		{
			include(IP_ROOT_PATH . 'includes/class_auth_admin.' . PHP_EXT);
		}
	...
	*/
	function table_index_add($table_name, $index_name = '', $column = array())
	{
		...
		// MIGHTY GORGON - BEGIN
		// Mighty Gorgon: To be checked!
		// remove index length if we are before 3.0.8 the feature (required for some types when using MySQL4) was added in that release (ticket PHPBB3-8944)
		/*
	...
	*/
	function create_table_sql($table_name, $table_data, $dbms = '')
	{
		...
		$supported_dbms = array('mysql_40', 'mysql_41');
		...
		foreach ($table_data['COLUMNS'] as $column_name => $column_data)
		{
			...
			if (isset($column_data[2]))
			{
				...
				elseif ($dbms === 'mysql_41' && $column_data[2] == 'true_sort')
				{
