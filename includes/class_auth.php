...
class auth
{
	...
	*/
	function acl(&$user_data)
	{
		...
		if (!trim($user_data['user_permissions']))
		{
			$this->acl_cache($user_data);
		}

		// Fill ACL array
		$this->_fill_acl($user_data['user_permissions']);
	...
	*/
	function acl_cache(&$user_data)
	{
		...
		$hold_ary = $this->acl_raw_data_single_user($user_data['user_id']);
		...
		if ($hold_str)
		{
			$user_data['user_permissions'] = $hold_str;

			$sql = 'UPDATE ' . USERS_TABLE . "
				SET user_permissions = '" . $db->sql_escape($user_data['user_permissions']) . "',
					user_perm_from = 0
	...
	*/
	function acl_raw_data_single_user($user_id)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			elseif (!empty($this->role_cache[$row['auth_role_id']]))
			{
