...
class p_master
{
	...
	function list_modules($p_class)
	{
		...
		foreach ($this->module_cache['modules'] as $key => $row)
		{
			...
			$module_row = array(
				...
				'lang' => ($row['module_basename'] && function_exists($lang_func)) ? $lang_func($row['module_mode'], $row['module_langname']) : (!empty($lang[$row['module_langname']]) ? $lang[$row['module_langname']] : (!empty($user->lang[$row['module_langname']]) ? $user->lang[$row['module_langname']] : $row['module_langname'])),
	...
	*/
	function set_active($id = false, $mode = false)
	{
		...
		foreach ($this->module_ary as $row_id => $item_ary)
		{
			...
			elseif (($item_ary['cat'] && ($item_ary['id'] === (int) $id)) || (($item_ary['parent'] === $category) && $item_ary['cat']))
			{
		...
		*/
		...
		if (!class_exists("{$this->p_class}_$this->p_name"))
		{
			...
			else
			{
				...
				{
					$this->module->u_action = IP_ROOT_PATH . (($user->page['page_dir']) ? $user->page['page_dir'] . '/' : '') . $user->page['page_name'];
				}
	...
	*/
	...
	{
		return $this->module->tpl_name . '.tpl';
	}
