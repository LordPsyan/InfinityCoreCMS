...
if(isset($_POST['edit']) && !defined('DEMO_MODE'))
{
	...
	foreach($update as $var => $value)
	{
		if(isset($config[$var]))
		{
			$sql = "UPDATE " . CONFIG_TABLE . " SET config_value='" . $db->sql_escape($value) . "' WHERE config_name='" . $var . "'";
		}
		else
		{
			$sql = "INSERT INTO " . CONFIG_TABLE . " (config_name, config_value) VALUES ('" . $var . "', '" . $db->sql_escape($value) . "')";
		}
		$db->sql_query($sql);
		$config[$var] = $value;
	}
