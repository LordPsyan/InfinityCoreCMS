...
* Include footer file
*/
include(IP_ROOT_PATH . 'includes/ctracker/engines/ct_footer.' . PHP_EXT);

/*
* Template
*/
$template->set_filenames(array('ct_body' => ADM_TPL . 'acp_footer_config.tpl'));
...
{
	set_config('ctracker_footer_layout', intval($_POST['ctracker_footer_layout']));
...
for($i = 1; $i <= 8; $i++)
{
	$template->assign_block_vars('footer_output', array(
		...
		'S_SELECT' => $i,
		'S_SELECTED' => ($i == $config['ctracker_footer_layout'])? ' checked="checked"': ''
		)
	);
}
