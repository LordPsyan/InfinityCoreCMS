...
{
	$config_data = array(
		'allow_ext_rating' => array('default' => 0, 'lang' => $lang['Allow_Detailed_Ratings_Page']),
		'rating_max' => array('default' => 10, 'lang' => $lang['Max_Rating']),
		'allow_rerate' => array('default' => 0, 'lang' => $lang['Allow_Users_To_ReRate']),
		'check_anon_ip_when_rating' => array('default' => 1, 'lang' => $lang['Check_Anon_IP']),
		'min_rates_number' => array('default' => 5, 'lang' => $lang['Min_Rates']),
		'index_rating_return' => array('default' => 10, 'lang' => $lang['Main_Page_Number']),
		'large_rating_return_limit' => array('default' => 10, 'lang' => $lang['Big_Page_Number']),
		'header_rating_return_limit' => array('default' => 10, 'lang' => $lang['Header_Page_Number']),
	);

	//Update config values if needed
	foreach ($config_data as $config_name => $c_data)
	{
		$config_value = request_var($config_name, $c_data['default']);
		if($config[$config_name] != $config_value)
		{
			set_config($config_name, $config_value, false);
			$admin_message .= '<br />' . $lang['Update'] . ':&nbsp;&nbsp;&nbsp;' . $c_data['lang'];
		}
	}
	...
	$rating_max = '<input class="post" type="text" size="3" maxlength="3" name="rating_max" value="' . $config['rating_max'] . '" />';
	...
	$check_anon_ip_when_rating = '<input type="radio" name="check_anon_ip_when_rating" value="1" ' . $check_anon_ip_yes . ' /> ' . $lang['Yes'] . '&nbsp;&nbsp;<input type="radio" name="check_anon_ip_when_rating" value="0" ' . $check_anon_ip_no . ' /> ' . $lang['No'];
	$min_rates_number = '<input class="post" type="text" size="5" maxlength="10" name="min_rates_number" value="' . $config['min_rates_number'] . '" />';
	$index_rating_return = '<input class="post" type="text" size="5" maxlength="10" name="index_rating_return" value="' . $config['index_rating_return'] . '" />';
	$large_rating_return_limit = '<input class="post" type="text" size="5" maxlength="10" name="large_rating_return_limit" value="' . $config['large_rating_return_limit'] . '" />';
	$header_rating_return_limit = '<input class="post" type="text" size="5" maxlength="10" name="header_rating_return_limit" value="' . $config['header_rating_return_limit'] . '" />';
